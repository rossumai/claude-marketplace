...
- **This is a PUBLIC repo — no customer identity, ever.** Customer names, real record values (addresses, IDs, names, labels), org IDs/hostnames, and customer-specific collection or dataset names must not appear in pack content, commit messages, PR titles, PR bodies, or **branch names**. Debugging context pasted into a session is internal; anything derived from it that lands here is published — that boundary crossing needs a deliberate sanitization pass, because it does not happen on its own. Sanitize to "a production deployment", `<known-correct-id>`-style placeholders, invented illustrative addresses, and generic collection descriptions. **Keep** the measurements, mechanisms and evidence tables — those carry all the value and none of the identity; a lesson never needs the customer's name to be useful. Before pushing, grep the staged diff *and* the commit messages for customer tokens, and check the branch name too. This has already gone wrong once: a PR shipped with a customer name in its title, body, two commit messages, real ship-to addresses and six customer collection names. Notification emails to repo watchers are unrecallable and a closed PR cannot be deleted without GitHub Support, so the only real defence is not writing it in the first place.
