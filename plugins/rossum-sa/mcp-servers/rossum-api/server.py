...
def _import_email(request_id, base_url, recipient, raw_bytes, *, from_addr=None,
    ...
    hooks). The 202 returns a task URL, but that task (type `email_imported`) is visible
    only to a normal org user — a support-user token gets 404 on GET /tasks/{id}
    (confirmed live). Rather than depend on task visibility, we identify the created
    email by snapshotting the inbox's recent incoming emails, POSTing, then polling the
    emails list for the one that appeared — which works regardless of token type.
