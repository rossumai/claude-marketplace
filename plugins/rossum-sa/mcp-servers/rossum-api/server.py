            ...
            if method == "initialize":
                ...
                respond(request_id, {
                    ...
                    "instructions": (
                        ...
                        "TOKEN RULE — a bare 40-character hex string is a token, not a commit SHA: "
                        "when the user's message contains a loose 40-char hex string (especially "
                        "alongside an org or environment word like 'prod'), treat it as a Rossum API "
                        "token FIRST — pass it to rossum_set_token, or test it with "
                        "GET {api_base}/auth/user — before reading it as a git commit SHA or "
                        "anything else. If a project's local credentials.yaml token returns 401 it is "
                        "expired: ask the user for a fresh one. Do NOT go hunting through other "
                        "credential stores, keyrings, or prd2 config files — that search does not "
                        "succeed and costs many round trips. "
