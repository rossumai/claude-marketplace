            ...
            if method == "initialize":
                ...
                respond(request_id, {
                    ...
                    "instructions": (
                        ...
                        "\n\n"
                        "EDITING RULE — Python lives in .py files, NEVER in JSON. This is repeatedly violated, so be deliberate.\n"
                        "\n"
                        "Formula fields (schema datapoints with ui_configuration.type='formula'):\n"
                        "  • Canonical source: `<queue_dir>/formulas/<field_id>.py`\n"
                        "  • The `formula` property inside schema.json is auto-generated by `prd2 push` from the .py.\n"
                        "  • To CHANGE a formula: edit the .py file. Do NOT touch `formula:` in schema.json.\n"
                        "  • To CREATE a new formula field: (1) write `formulas/<field_id>.py` with the Python first, "
                        "(2) add the datapoint definition in schema.json WITHOUT a `formula` property — `prd2 push` fills it in.\n"
                        "\n"
                        "Function-hook code (type='function'):\n"
                        "  • Canonical source: `<hook_name>_[id].py` (sibling to the hook JSON file).\n"
                        "  • The `code` field inside the hook JSON is auto-generated by `prd2 push`. Edit only the .py.\n"
                        "\n"
                        "Forbidden actions for code changes (these silently get overwritten or break prd2 sync):\n"
                        "  ✗ Editing the `formula` value inside schema.json (incl. adding it inline when creating a new field).\n"
                        "  ✗ Editing the `code` value inside a function-hook JSON.\n"
                        "  ✗ Calling rossum_patch_schema or rossum_patch_hook to push formula/code changes.\n"
                        "  ✗ Workarounds via curl PATCH that bypass prd2.\n"
                        "\n"
                        "If `prd2 push` is broken in the environment: investigate and fix the prd2 issue (e.g. asyncio "
                        "TTY errors can be worked around with `prd2 push -c -m '...'` or running from a real terminal). "
                        "Do NOT bypass via direct API patches — the .py file remains the source of truth and the next pull "
                        "will revert silent JSON-only edits.\n"
                        "\n"
                        "Allowed JSON edits in schema.json: everything that is NOT code — field IDs, labels, types, "
                        "constraints, ui_configuration, options, can_export, hidden, multivalue/tuple structure, ordering."
                    ),
                })
