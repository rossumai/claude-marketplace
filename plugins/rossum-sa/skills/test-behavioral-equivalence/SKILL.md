...
4. **Search index parity** — for any MDH query containing `$search` with a named `index` (e.g., `"index": "search_entities_v1"`), the named Atlas Search index must be **declared** on the target dataset. Use `data_storage_list_search_indexes` against both source and target and compare index names, field mappings, and analyzers — plus `queryable` / `status`, which the V2 API now reports directly (`FAILED` means the declaration is valid but the engine could not build it). Differently-named indexes cause `$search` stages to silently return no results → fuzzy-match queries fall through → match outputs end up empty. **This is the most common environment-clone drift cause and the hardest to diagnose** — the MDH hook log shows `status: completed` (no error), the data is present, but the result is empty because the query names an index the target does not declare. Search indexes are now durable (MDH reconciles the registry onto the collection), so this check is about **naming and mapping conformance across the two envs**, not about an index having disappeared — an env-suffix divergence such as `search_suppliers_v1` vs `search_suppliers_dev_v1` survives durability untouched. Always check this when:
