...
- **`POST /v1/emails/import` quirks** [live]: it rejects messages without a `Date` header / with bare-LF line endings as HTTP 400 "Invalid e-mail format", and it does **not** thread imported emails by `In-Reply-To` (`parent` stays null) — relevant for anyone testing reply handling via this endpoint, and the reason `skip_if_email_is_a_reply`'s semantics remain unverified. The 202 response's task URL (type `email_imported`) is retrievable by a **normal org user** (200, `running`→`succeeded`), but returns **404 for a support-user token** — even though the *same* support token can retrieve the `upload_created` tasks it creates. **Confirmed Rossum-side bug** (acknowledged July 2026, no fix ETA): the `email_imported` task is created in the **support user's default organization**, not the target org, so a support-user token 404s on it when querying the target org; a normal org user (whose default org *is* the target) reads it fine. Practical guidance: read the task as a normal org user, or just poll the **email object** (e.g. `GET /emails?to=<inbox>&type=incoming&ordering=-created_at`), which confirms the import regardless of token type. The **`rossum_import_email` MCP tool** wraps this endpoint and is the easiest way to simulate an inbound email for testing the converter — when you let it build the message from `subject`/`body_html`/`attachments` it emits CRLF + a `Date` header for you, sidestepping the 400; pass `raw_message_path` only when you need a hand-crafted `.eml` (then ensure CRLF + Date yourself).
