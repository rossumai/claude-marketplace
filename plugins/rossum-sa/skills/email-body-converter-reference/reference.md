...
- **`POST /v1/emails/import` quirks** [live]: it rejects messages without a `Date` header / with bare-LF line endings as HTTP 400 "Invalid e-mail format", and it does **not** thread imported emails by `In-Reply-To` (`parent` stays null) — relevant for anyone testing reply handling via this endpoint, and the reason `skip_if_email_is_a_reply`'s semantics remain unverified. The 202 response's task URL (type `email_imported`) is retrievable by a **normal org user** (200, `running`→`succeeded`), but returns **404 for a support-user token** — so if `GET /tasks/{id}` 404s, check the token's user before assuming a platform issue (confirmed live, July 2026). Either way, polling the **email object** (e.g. `GET /emails?to=<inbox>&type=incoming&ordering=-created_at`) confirms the import regardless of token type. The **`rossum_import_email` MCP tool** wraps this endpoint and is the easiest way to simulate an inbound email for testing the converter — when you let it build the message from `subject`/`body_html`/`attachments` it emits CRLF + a `Date` header for you, sidestepping the 400; pass `raw_message_path` only when you need a hand-crafted `.eml` (then ensure CRLF + Date yourself).
