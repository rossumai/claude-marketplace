...
**How the copy works.** Two techniques, and they answer different questions.

- **Fresh run** — `rossum_refire_annotation mode="reupload"` re-uploads the source PDF and returns a **new annotation ID** (`_refire.target_annotation_id`); the original is never touched. Re-upload re-runs OCR + extraction from scratch, so the result is a *fresh run of the same document*, not a content-identical clone — manual corrections on the original are not carried over. This is what you want for the usual "does my fix make a fresh run come out right?" test.
- **Content-identical clone** — `rossum_copy_annotation` clones the annotation with its content intact, including `validation_sources` human marks and datapoint positions. Use it when your assertions depend on specific human-entered values, which `reupload` would discard. **Copy into the source annotation's own queue** — pass its queue id as `target_queue` (the parameter is required) and leave `target_status` alone. Do **not** reach for a different queue here: cross-queue, only `target_status="importing"` retargets the schema, and `importing` re-runs the import chain, which re-extracts the document and discards exactly the human-entered values you copied it to preserve. Cross-queue you get the target schema **or** the original content, never both. Note the copy inherits `confirmed_at`/`confirmed_by`, so it *looks* confirmed although it never was, and it does **not** inherit labels or relations.

Either way, iterate against the new ID from then on. Do not use `reextract` for this — it re-imports the original in place and replaces its content.

**Cleanup.** When the loop ends, if you created a copy, **offer to delete it** (don't auto-delete): use `rossum_delete_annotation` (soft-delete by default; add `purge=true` for a permanent, irreversible purge) — a write, so it passes the hard-gate. For a simple status flip without purging, `rossum_patch_annotation` with `status="deleted"` still works. Record the copy's ID in the task list so it doesn't get orphaned. If the copy came from a `rossum_upload_document` or `reupload` call and you need to track the async upload state, `rossum_get_task` polls the task object directly (no-follow GET, surfaces the status behind the `/tasks` 303 redirect).
