...
- **Content-identical clone** — `rossum_copy_annotation` clones the annotation with its content intact, including `validation_sources` human marks and datapoint positions. Use it when your assertions depend on specific human-entered values, which `reupload` would discard. Copy within the source queue, or pass `target_queue` **with** `target_status="importing"` so the copy picks up that queue's schema (any other status leaves it on the source schema — a broken hybrid across queues). Note the copy inherits `confirmed_at`/`confirmed_by`, so it *looks* confirmed although it never was, and it does **not** inherit labels or relations.
